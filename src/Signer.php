...
class Signer
{
    ...
    public static function sign($request,$signOptions)
    {
        ...
        if (is_null($service))
            throw new SignerException('Service is required');

        if (is_null($accessKey))
            throw new SignerException('Access key is required');

        if (is_null($secretKey))
            throw new SignerException('Secret key is required');

        if (is_null($region))
            throw new SignerException('Region key is required');

        if (is_null($host))
            throw new SignerException('Host key is required');

        if (is_null($method))
            throw new SignerException('Method key is required');
        ...
        $headers = array_merge($canonicalHeaders,['Authorization'=>$authorizationHeader]);
